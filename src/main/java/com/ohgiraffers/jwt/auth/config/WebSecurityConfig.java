...
public class WebSecurityConfig {
    ...
    public WebSecurityCustomizer webSecurityCustomizer(){
        ...
        // 특정 요청에 대해 spring security filter chain을 건더뛰도록 하는 역할
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        http
                ...
                // carf(create-Site-Request-Rorgray)
                // RESTAOI 홋은 JWT 기반 인증에서는 세션을 사용하지 않아서 보호를 하지 않아도 됨
                .csrf(AbstractHttpConfigurer::disable)
                // 어플리케이션의 session 형태를 비저창 모드로 동작하게 함
                .sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))

                // 기존 formlogin을 사용하지 않으므로 비활성화
                .formLogin(AbstractHttpConfigurer::disable)

                // http 기본인증(JWT 사용할 것이므로) 비활성화
                .httpBasic(AbstractHttpConfigurer::disable)

                // 사용자가 입력한 아이디 패스워드를 전달받아 로그인을 직접적으로 수행하는 필터
                // 인증시 (successHandler를 통해) 토큰을 생성해서 header로 전달하고
                // 실패시 (failHandler를 통해) 실패 이유를 달아서 응답한다.
                .addFilterBefore(customAuthenticationFilter(), UsernamePasswordAuthenticationFilter.class)

                // header에  token이 담겨져 있을 경우 인가처리를 해주는 필터
    ...
    public CustomAuthenticationFilter customAuthenticationFilter(){
        ...
        // /login으로 post 요청이 들어오면 필터가 동작한다.
        ...
        // 필터의 모든 속성 설정을 완려했을 대
        // 올바르게 설정되어있는지 확인하는 역할의 매소드
        customAuthenticationFilter.afterPropertiesSet();

        // 완성된 CustomAuthentication으로 드러안다.
        return customAuthenticationFilter;
    }
